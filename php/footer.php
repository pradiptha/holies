<!DOCTYPE html>
<html>

<head>
	<title></title>
	<meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
	<meta name="description" content="">
	<meta name="author" content="">
	<!-- Bootstrap core CSS -->
	<link href="../vendor/bootstrap/css/bootstrap-modified.min.css" rel="stylesheet">
	<link href="https://fonts.googleapis.com/icon?family=Material+Icons" rel="stylesheet">
	<link rel="stylesheet" href="https://use.fontawesome.com/releases/v5.8.2/css/all.css" integrity="sha384-oS3vJWv+0UjzBfQzYUhtDYW+Pj2yciDJxpsK1OYPAYjqT085Qq/1cq5FLXAZQ7Ay" crossorigin="anonymous">
	<link rel="stylesheet" type="text/css" href="../vendor/holies/css/footer-fix.css">
	<style>
		.vl {
			border-left: 3px solid green;
			border-radius: 20px;
			height: 136px;
			position: absolute;
			left: 100%;
			margin-left: 5px;
			top: 42px;
		}
	</style>
</head>

<body>
	<footer class="footer-top">
		<div class="container">
			<div class="row ">
				<div class="col-md-3 segment-one ">
					<div class="f2">
						<ul style="list-style-type: none;">
							<li><a class="text-white" href="../php/bantuan.php">Buka Bantuan</a></li>
							<li> <a class="text-white" href="../php/pandubelanja.php">Panduan Belanja</a></li>
							<li><a class="text-white" href="../php/panduseller.php"> Panduan Seller</a></li>
							<li><a class="text-white" href="../php/faq.php"> FAQ</a></li>
						</ul>
					</div>
				</div>
				<div class="col-md-7 segment-two">
					<p class="text-center text-success font-weight-bold"> Temukan kami disini </p>
					<div class="row justify-content-center">
						<div class="col-3 ">
							<img style="width :20%" src="../img/fb.png" class="rounded mx-auto d-block">
							<div class="media-body">
								<p class="text-center"><a href="#" class="text-success">Facebook</a></p>
							</div>
						</div>
						<div class="col-3">
							<img style="width :20%" src="../img/ig.png" class="rounded mx-auto d-block">
							<div class="media-body">
								<p class="text-center"><a href="#" class="text-success">Instagram</a></p>
							</div>
						</div>
						<div class="col-3">
							<a href="#" class="text-success">
								<img style="width :20%" src="../img/tw.png" class="rounded mx-auto d-block">
								<div class="media-body">
									<p class="text-center">Twitter</p>
								</div>
							</a>
						</div>
					</div>
				</div>
			</div>
		</div>
	</footer>
	<footer class="bg-success p-1">
		<p class="font-weight-regular text-center text-white mt-3">2019.All Rights Reserverd by PT. Holies Indonesia </p>
	</footer>

</body>

</html>